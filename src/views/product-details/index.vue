<template>
  <div class="prodetail">
    ...
    <van-swipe :autoplay="3000" @change="onChange">
      <van-swipe-item v-for="(image) in productImages" :key="image.file_id">
        <img :src="image.external_url"/>
      </van-swipe-item>

      <template #indicator>
        <div class="custom-indicator">{{ current + 1 }} / {{ productImages.length }}</div>
      </template>
    </van-swipe>
    ...
    <div class="comment">
      ...
      <div class="comment-list">
        <div class="comment-item" v-for="item in commentsList" :key="item.comment_id">
          <div class="top">
            <img :src="item.user.avatar_url || defaultUserAvatar" alt="">
            <div class="name">{{ item.user.nick_name }}</div>
            <van-rate :size="16" :value="item.score/10" color="#ffd21e" void-icon="star" void-color="#eee"/>
          </div>
          <div class="content">
            {{ item.content }}
          </div>
    ...
    <!-- 商品描述 -->
    <div class="desc" v-html="productDetail.content">
    </div>
...
export default {
  ...
  computed: {
    productImages () {
      return this.productDetail.goods_images || []
    },
    commentsList () {
